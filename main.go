...
func startTranscode(stream *api.Stream, output api.Output) {
	...
	if output.Variant == "src" {
		cmd = exec.Command("ffmpeg", "-hide_banner", "-rtmp_enhanced_codecs", "hvc1,av01", "-i", input,
			...
			"-hls_segment_filename", utils.Config.Cache.Hostname+"/transcode/"+stream.User.StreamKey+"_"+output.Variant+"/%d.m4s", utils.Config.Cache.Hostname+"/transcode/"+stream.User.StreamKey+"_"+output.Variant+"/index.m3u8")
	} else {
		cmd = exec.Command("ffmpeg", "-hide_banner", "-rtmp_enhanced_codecs", "hvc1,av01", "-i", input,
